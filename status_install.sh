...
Service_Server_Status_client(){
	if [[ ${release} = "centos" ]]; then
		if ! wget -N --no-check-certificate "https://gitee.com/zcyso/ServerStatusPlus/raw/master/service/client_centos" -O /etc/init.d/status; then
		...
		if ! wget -N --no-check-certificate "https://gitee.com/zcyso/ServerStatusPlus/raw/master/service/client_debian" -O /etc/init.d/status; then
			echo -e "${Error} ServerStatusPlus 客户端服务管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/status
