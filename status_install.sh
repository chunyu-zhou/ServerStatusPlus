...
Read_config_client(){
	...
	wget -N --no-check-certificate -O "/usr/local/ServerStatusPlus/status-plus-client.py" "https://gcore.jsdelivr.net/gh/chunyu-zhou/ServerStatusPlus/status-plus-client.py"
...
Service_Server_Status_client(){
	if [[ ${release} = "centos" ]]; then
		if ! wget -N --no-check-certificate "https://gcore.jsdelivr.net/gh/chunyu-zhou/ServerStatusPlus/service/client_centos" -O /etc/init.d/status; then
		...
		if ! wget -N --no-check-certificate "https://gcore.jsdelivr.net/gh/chunyu-zhou/ServerStatusPlus/service/client_debian" -O /etc/init.d/status; then
			echo -e "${Error} ServerStatusPlus 客户端服务管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/status
